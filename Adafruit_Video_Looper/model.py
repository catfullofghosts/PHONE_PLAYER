...
class Playlist:
    ...
    def find_by_key(self, key: str) -> Optional[Movie]:
        """Find a movie by keypad label, filename, or title (without extension)."""
        if not key:
            return None

        for movie in self._movies:
            if movie.filename == key or movie.title == key:
                return movie

        key_lower = key.lower()
        for movie in self._movies:
            if movie.title.lower() == key_lower:
                return movie
            if movie.filename.lower().startswith(key_lower + "."):
                return movie
    ...
    def set_next(self, thing: Union[Movie, str, int]):
        if isinstance(thing, Movie):
            ...
            elif thing[0:1] in ("+", "-") and self._index is not None:
                self._next = self._movies[(self._index + int(thing)) % self.length()]
            else:
                self._next = self.find_by_key(thing)
        elif isinstance(thing, int):
            if 0 <= thing < self.length():
