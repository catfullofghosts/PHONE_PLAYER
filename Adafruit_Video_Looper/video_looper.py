...
from .matrix_keypad import create_keypad
...
class VideoLooper:
    ...
    def __init__(self, config_path):
        ...
        self._fgcolor = list(map(int, self._config.get('video_looper', 'fgcolor')
                                             .translate(str.maketrans('','', ','))
                                             .split()))
        self._headless = self._config.getboolean('video_looper', 'headless', fallback=False)
        if self._headless:
        ...
        if self._config.getboolean('keypad', 'enabled', fallback=False):
            try:
                ...
                self._print("Matrix keypad enabled")
            except Exception as err:
                self._keypad = None
                self._print(f"Matrix keypad setup failed: {err}")
    ...
    def _handle_keypad_control(self):
        if self._keypad is None:
            return

        key = self._keypad.get_key()
        if key is None:
            return
